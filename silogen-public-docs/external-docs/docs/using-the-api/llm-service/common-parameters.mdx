...
| collection              | No       | List                 | **List of collections for the RAG component**. Usually only one. Allowed values are provided by your SiloGen contact. Or, you can use the [RAG Document Collection Service API](../rag-document-collection-service.mdx) to list available collections.     |
